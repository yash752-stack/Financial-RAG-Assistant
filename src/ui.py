...
from src.config import configure_environment, configure_page, ensure_session_defaults
...
# ─────────────────────────────────────────────────────────────────────────────
# TOP ACTION BAR — iframe visual + hidden real st.buttons triggered by JS
# ─────────────────────────────────────────────────────────────────────────────
...
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  #upload-btn::before{{
    ...
    background:radial-gradient(ellipse at 35% 35%,rgba(56,189,248,.07) 0%,transparent 55%);
    pointer-events:none;
  }}
  /* Inner corner frame */
  #upload-btn::after{{
    ...
    border-left-color:rgba(56,189,248,.16);
    pointer-events:none;
  ...
  #upload-btn:hover{{
    ...
    box-shadow:
      ...
      0 0 70px rgba(0,160,240,.6),
      0 20px 50px rgba(0,0,0,.65),
      inset 0 1px 0 rgba(56,189,248,.18);
  }}
  #upload-btn:hover::after{{border-top-color:rgba(56,189,248,.65);border-left-color:rgba(56,189,248,.42)}}
  #upload-btn.active{{
    border-color:rgba(56,189,248,.92);
    transform:translateY(-7px) scale(1.04);
    box-shadow:
      0 0 0 2px rgba(56,189,248,.32),
      0 0 90px rgba(0,160,240,.7),
      0 20px 50px rgba(0,0,0,.7),
      inset 0 1px 0 rgba(56,189,248,.22);
  }}
  /* Scan line sweep */
  @keyframes scan{{
    0%{{top:-5%;opacity:0}}10%{{opacity:.45}}90%{{opacity:.3}}100%{{top:105%;opacity:0}}
  }}
  ...
  .up-shaft{{
    ...
    border-radius:2px;box-shadow:0 0 12px rgba(56,189,248,.55);
    position:relative;
  }}
  .up-shaft::before,.up-shaft::after{{
    content:'';position:absolute;top:1px;
    width:clamp(8px,1.5vw,12px);height:2.5px;
    background:#38bdf8;border-radius:2px;
    box-shadow:0 0 8px rgba(56,189,248,.55);
  }}
  .up-shaft::before{{left:calc(-1*clamp(8px,1.5vw,12px) + 1px);transform:rotate(45deg);transform-origin:right center}}
  .up-shaft::after {{right:calc(-1*clamp(8px,1.5vw,12px) + 1px);transform:rotate(-45deg);transform-origin:left center}}
  .up-base{{
    width:clamp(24px,4vw,36px);height:2.5px;
    background:linear-gradient(90deg,transparent,rgba(56,189,248,.75),transparent);
    border-radius:2px;
  }}
  .up-title{{
    font-size:clamp(10px,1.4vw,14px);letter-spacing:4px;
    text-transform:uppercase;color:rgba(56,189,248,.85);
    text-shadow:0 0 14px rgba(56,189,248,.4);
    font-weight:700;
  }}
  .up-sub{{
    font-size:clamp(7px,0.9vw,10px);letter-spacing:2px;
  ...
  #globe{{
    ...
    border:1.5px solid rgba(56,189,248,.38);
    position:relative;cursor:pointer;overflow:hidden;
    box-shadow:
      0 0 0 4px rgba(56,189,248,.05),
      0 0 0 9px rgba(56,189,248,.025),
      0 0 50px rgba(0,110,170,.45),
      0 10px 36px rgba(0,0,0,.75);
  ...
  #globe:hover{{
    ...
    box-shadow:
      ...
      0 0 0 10px rgba(56,189,248,.05),
      0 0 80px rgba(0,150,230,.65),
  ...
  #globe.active{{
    ...
    box-shadow:
      ...
      0 0 0 12px rgba(56,189,248,.1),
      0 0 110px rgba(0,160,240,.78),
  ...
  #globe-canvas{{position:absolute;inset:0;border-radius:50%;pointer-events:none}}
  #globe-info{{
    position:absolute;inset:0;
    display:flex;flex-direction:column;align-items:center;justify-content:center;
    z-index:5;pointer-events:none;gap:3px;padding:10px;
  }}
  .g-flag{{font-size:clamp(22px,3.5vw,34px);line-height:1;
    filter:drop-shadow(0 1px 5px rgba(0,0,0,.9));
    transition:opacity .25s ease,transform .25s ease}}
  .g-name{{font-size:clamp(8px,1vw,11px);letter-spacing:2px;
    color:rgba(56,189,248,.85);text-transform:uppercase;
    text-shadow:0 0 8px rgba(0,0,0,.9);
    transition:opacity .25s ease,transform .25s ease}}
  .g-val{{font-family:'Cormorant Garamond',serif;
    font-size:clamp(16px,2.2vw,24px);font-weight:300;
    color:#e8f6ff;line-height:1.1;
    text-shadow:0 0 16px rgba(56,189,248,.35);
    transition:opacity .25s ease,transform .25s ease}}
  .g-chg{{font-size:clamp(10px,1.2vw,13px);font-weight:700;letter-spacing:1px;
    transition:opacity .25s ease,transform .25s ease}}
  .g-chg.up{{color:#4ade80;text-shadow:0 0 12px rgba(74,222,128,.65)}}
  .g-chg.dn{{color:#f87171;text-shadow:0 0 12px rgba(248,113,113,.65)}}
...
<div id="upload-btn" onclick="doUpload()">
  <div class="scan"></div>
  <div class="up-icon">
    <div class="up-shaft"></div>
    <div class="up-base"></div>
  </div>
...
<div id="sim-wrap">
  <div id="globe" onclick="doPortfolio()">
...
if({_pf_open_js}) document.getElementById('globe').classList.add('active');
if({_up_open_js}) document.getElementById('upload-btn').classList.add('active');
...
function drawChart(col){{
  ...
  var pad=12,bot=h-10,top=16;
  var mn=Math.min.apply(null,chartData),mx=Math.max.apply(null,chartData),rng=mx-mn||8;
  var rgb=h2r(col);
  ...
  gr.addColorStop(0,'rgba('+rgb+',0.22)');
  gr.addColorStop(1,'rgba('+rgb+',0)');
  ctx.beginPath();ctx.moveTo(px(0),bot);
  chartData.forEach(function(v,i){{ctx.lineTo(px(i),py(v));}});
  ctx.lineTo(px(chartData.length-1),bot);ctx.closePath();
  ctx.fillStyle=gr;ctx.fill();
  // Line
  ctx.beginPath();
  chartData.forEach(function(v,i){{i?ctx.lineTo(px(i),py(v)):ctx.moveTo(px(i),py(v));}});
...
function showMarket(i){{
  ...
  var els=[document.getElementById('g-flag'),document.getElementById('g-name'),
           document.getElementById('g-val'),document.getElementById('g-chg')];
  els.forEach(function(e){{e.classList.add('fade-out');}});
  document.getElementById('globe').style.borderColor=m.col+'99';
  genChart(m.up);resize();drawChart(m.col);
  setTimeout(function(){{
    ...
    var c=document.getElementById('g-chg');
    c.textContent=(m.up?'▲ ':'▼ ')+m.chg;
    c.className='g-chg '+(m.up?'up':'dn');
    els.forEach(function(e){{e.classList.remove('fade-out');}});
  }},260);
}}

// Click handlers — find the hidden Streamlit buttons in the PARENT frame and click them
function clickParentBtn(text){{
  try{{
